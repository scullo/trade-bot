...
class CryptoPaymentGateway:
    ...
    Otomatik USDT (TRC20 / BEP20) Odeme Dogrulama, Anti-Replay Kalkanı
    ve 30 Gunluk Otonom Lisans Uzatma Motoru.
    """
    ...
    def validate_tx_hash_format(self, tx_hash: str, network: str = "TRC20") -> bool:
        ...
        if re.match(r'^(0x)?[a-fA-F0-9]{64}$', clean):
            return True
        # Tron base58 transaction id formati
        if len(clean) >= 64:
            return True
        return False

    async def verify_and_activate_payment(self, user_id: int, plan_type: str, tx_hash: str, network: str = "TRC20") -> tuple:
        """
        ...
        # 1. KATMAN: ANTI-REPLAY / ANTI-DOUBLE SPEND DENETIMI
        ...
        # 2. KATMAN: PLAN FIYATLANDIRMA KONTROLU
        settings = self.db.get_payment_settings()
        price_expected = settings.get("price_vip", 199.0) if plan_type == "VIP" else settings.get("price_pro", 69.0)
        ...
        try:
            with self.db.get_connection() as conn:
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, 'VERIFIED')
                """, (user_id, plan_type, clean_tx, network, price_expected, admin_wallet, receipt_id))

                # 3.2 Kullanicinin aboneligini 30 gun uzat
                # Mevcut aktif aboneligi varsa bitis tarihine +30 gun ekle, yoksa su andan +30 gun baslat
                cursor.execute("""
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, 'ACTIVE', ?)
                """, (user_id, plan_type, now_str, new_expires_str, receipt_id))
                ...
                return True, f"🎉 Ödeme Başarıyla Doğrulandı! {plan_type} üyeliğiniz 30 gün boyunca ({new_expires_str} tarihine kadar) aktif edildi.", receipt_data
