...
class StrategyEngine:
    ...
    async def _handle_open(self, symbol: str, side: str, entry_price: float, reason: str, soft_stop: float, hard_stop: float, tp1: float, tp2: float = None, trade_type: str = "BREAKOUT", snapshot_levels: dict = None, setup_id: str = "", confluence_list: list = None):
        ...
        # 1b. Judas Swing Dip Tuzağı: Fiyat Asya Dibini veya PDL'yi delip hemen ardından üstüne çıkmışsa dip breakdown SHORT VETO!
        if side == "SHORT" and is_breakout:
            ref_low = min(pdl_lvl, asia_low_lvl) if (pdl_lvl > 0 and asia_low_lvl > 0) else max(pdl_lvl, asia_low_lvl)
            if ref_low > 0 and entry_price <= ref_low * 1.005 and entry_price >= ref_low * 0.992:
                df_5m_chk = self.market_data.candles_5m.get(symbol, pd.DataFrame()) if self.market_data else pd.DataFrame()
                if not df_5m_chk.empty and len(df_5m_chk) >= 2:
                    prev_low = float(df_5m_chk['low'].iloc[-2])
                    prev_close = float(df_5m_chk['close'].iloc[-2])
                    if prev_low <= ref_low and prev_close > ref_low:
                        rej_msg = f"🛡️ Judas Swing Dip Tuzağı: Asya/PDL dibi (${ref_low:.4f}) iğnelenip içeri dönüldü (Bear Trap). Dip SHORT engellendi."
                        print(f">> [RED - JUDAS SWING] {symbol}: {rej_msg}")
                        self.log_rejection(symbol, reason, rej_msg)
                        return {"error": "JUDAS_SWING_LOW_BLOCKED"}
        ...
        if side == "LONG" and not is_breakout and ref_low > 0:
            ...
            if ref_high > 0:
                ...
                if not df_5m_chk.empty and len(df_5m_chk) >= 2:
                    ...
                    if prev_high >= ref_high and prev_close < ref_high:
                        if confluence_list is not None and isinstance(confluence_list, list):
                            confluence_list.append("Asya_PDH_Tepe_Süpürmesi_Teyitli")
        ...
        # ⚠️ KUANT DÜZELTMESİ: Boğa rejiminde Short tasfiyeleri tepe dönüşü DEĞİL, roket yakıtıdır (Short Squeeze Fuel). Teyit üretmesi engellenir!
        elif side == "SHORT" and not is_breakout:
            if "BOĞA" not in trend_regime and (short_liq_usd >= 10000.0 or (short_liq_usd >= 5000.0 and short_liq_usd > long_liq_usd * 1.4)):
                liq_confirmed = True
        ...
        # ── 2d. ALFA BOĞA PATLAMA & ROTASYON KALKANI (ALPHA SURGE & ROTATION SHORT SHIELD) ──
        # Piyasa genelinden pozitif ayrışan güçlü boğa coinlerine VEYA boğada geride kalıp aniden rotasyonla patlayan zayıf coinlere karşı SHORT açmak engellenir!
        if side == "SHORT":
        ...
        # 1b. Anlık 5M BTC Fışkırma Kalkanı: BTC son 5 dakikada sert yükseliyorsa altcoinlerde SHORT açma!
        if side == "SHORT" and btc_chg_5m >= 0.25:
            rej_msg = f"⚡ 5M BTC Fışkırma Kalkanı: Bitcoin son 5 dakikada %{btc_chg_5m:+.2f} sert boğa mumu atıyor. Yükselen rokete SHORT açılmadı."
            print(f">> [RED - BTC 5M FİŞKIRMA] {symbol}: {rej_msg}")
            self.log_rejection(symbol, reason, rej_msg)
            ...
            try:
                ...
                if depth_ok:
                    ...
                    if is_anchor_aged:
                        ...
                        elif side == "SHORT" and obi_wall_side == "ASK_WALL" and "BOĞA" not in trend_regime:
                            if confluence_list is not None and isinstance(confluence_list, list) and "🧱_Yaşlanmış_Kurumsal_Çapa_Duvarı" not in confluence_list:
                                confluence_list.append("🧱_Yaşlanmış_Kurumsal_Çapa_Duvarı")
    ...
    async def evaluate_candle_close(self, symbol: str, current_candle: dict, prev_candle: dict, levels: dict):
            ...
            if not is_warming_up:
                ...
                if is_breakout and hold_candles >= 4.0:
                    if max_mfe_seen < 1.50 and roe_raw <= -2.00:
                        record = await self._safe_close_position(
                            symbol, close_price,
                            f"⏱️ Kırılım İvmesizlik Kalkanı (20dk İvmelenmedi, ROE: %{roe_raw:+.1f})")
                        if record:
                            await self._notify_close(record, levels=levels)
                            self._cleanup_tracking(symbol)
                        return
        ...
        # ─────────────────────────────────────────────────────────────────
        # SETUP 11: PİVOT P / mPOC / TEPE AVWAP ALTTAN RETEST AYI REDDİ SHORT (Bearish Resistance Flip)
        # Fiyat Pivot P veya mPOC altında seyrederken, yukarı tepki verip bu seviyeyi alttan test eder
        # ve satıcı tepkisiyle (üst fitil / kırmızı kapanış) reddedilir → Trend yönünde yüksek R:R SHORT
        # ─────────────────────────────────────────────────────────────────
        resist_cands = [lvl for lvl in [p, mpoc, tepe_avwap] if lvl and lvl > 0]
        nearby_resists = [lvl for lvl in resist_cands if close_price < lvl and current_candle.get('high', close_price) >= lvl * 0.998 and lvl <= close_price * 1.020]

        if nearby_resists:
            resist_lvl = min(nearby_resists)
            resist_tag = "Pivot P" if resist_lvl == p else ("mPOC" if resist_lvl == mpoc else "Tepe AVWAP")
            ...
            if not struct_ok:
                ...
                if "BOĞA" in trend_regime or macro.get("btc_chg_1h", 0.0) > 0.15 or (coin_rs_score >= 0.5 and not is_bear_dump):
                    ...
                    if not is_seller_rejection:
                        ...
                        down_targets = [
                            lvl for lvl in [s3, below_npoc, mval, s4, s5]
                            if lvl and lvl <= close_price * 0.992
                        ]
                        down_targets.sort(reverse=True)
                        ...
                        await self._handle_open(
                            symbol=symbol, side="SHORT", entry_price=close_price,
                            reason=f"{resist_tag} Direnç Retest Reddi (Bearish Resistance Flip)",
                            soft_stop=soft_stop, hard_stop=hard_stop,
                            tp1=tp1_target, tp2=tp2_target, trade_type="SCALP",
        ...
        # ─────────────────────────────────────────────────────────────────
        # SETUP 12: DESTEK ÇÖKÜŞÜ SHORT (Aşağı nPOC / S3 Breakdown with Seller Wall)
        # Fiyat Aşağı nPOC veya S3 desteğini aşağı kırar ve tahtada satıcı duvarı baskındır
        # Düşen bıçağı tutmak yerine trend yönünde SHORT açılarak satıcı gücü arkaya alınır!
        # ─────────────────────────────────────────────────────────────────
        break_support = None
        support_type = ""
        if support_npoc and support_npoc > 0 and prev_close >= support_npoc and close_price < support_npoc:
            break_support = support_npoc
            support_type = "Aşağı nPOC"
        elif s3 and s3 > 0 and prev_close >= s3 and close_price < s3 and (close_price > s4 if s4 else True):
            break_support = s3
            support_type = "S3 Destek"

        if break_support:
            struct_ok, struct_reason = self.check_structural_invalidation(symbol, "SHORT", close_price, levels)
            if not struct_ok:
                self.log_rejection(symbol, f"SETUP 12 {support_type} Çöküşü", struct_reason)
            else:
                cvd_data = self.market_data.get_symbol_cvd(symbol) or {} if (self.market_data and hasattr(self.market_data, 'get_symbol_cvd')) else {}
                cvd_ratio = float(cvd_data.get('ratio_60s', 50.0))
                ...
                if not has_seller_pressure and vol_surge < 1.3:
                    ...
                    await self._handle_open(
                        symbol=symbol, side="SHORT", entry_price=close_price,
                        reason=f"{support_type} Destek Çöküşü (Bearish Support Breakdown)",
                        soft_stop=soft_stop, hard_stop=hard_stop,
        ...
        # ─────────────────────────────────────────────────────────────────
        # SETUP 13: S3 DİRENÇ RETESTİ VE AYI DEVAMI SHORT (S3 Resistance Flip)
        # Fiyat S3'ün altındayken (S3 kırılmışken) yukarı dönüp S3'ü alttan direnç olarak test eder
        # ve satıcı tepkisiyle geri itilir → Trend yönünde devam SHORT
        # ─────────────────────────────────────────────────────────────────
        if s3 and s3 > 0 and prev_close < s3 and current_candle.get('high', close_price) >= s3 * 0.998 and close_price < s3 and (close_price > s4 if s4 else True):
            struct_ok, struct_reason = self.check_structural_invalidation(symbol, "SHORT", close_price, levels)
            if not struct_ok:
                self.log_rejection(symbol, "SETUP 13 S3 Retest Reddi", struct_reason)
            else:
                c_high = current_candle.get('high', close_price)
                c_low = current_candle.get('low', close_price)
                c_open = current_candle.get('open', close_price)
                ...
                if not is_seller_rej:
                    ...
                    await self._handle_open(
                        symbol=symbol, side="SHORT", entry_price=close_price,
                        reason="S3 Direnc Retest Sekmesi (Resistance Flip)",
                        soft_stop=soft_stop, hard_stop=hard_stop,
                        tp1=tp1_target, tp2=tp2_target, trade_type="SCALP",
        ...
        # ─────────────────────────────────────────────────────────────────
        # SETUP 14: PİVOT P / mPOC / DİP AVWAP ÜSTTEN RETEST BOĞA SEKMESİ LONG (Bullish Support Flip)
        # Fiyat Pivot P, mPOC veya Dip AVWAP üzerinde seyrederken, aşağı sarkıp bu seviyeyi üstten test eder
        # ve alıcı tepkisiyle (alt fitil / yeşil kapanış) seker → Trend yönünde yüksek R:R LONG
        # ─────────────────────────────────────────────────────────────────
        support_cands = [lvl for lvl in [p, mpoc, dip_avwap] if lvl and lvl > 0]
        nearby_supports = [lvl for lvl in support_cands if close_price > lvl and current_candle.get('low', close_price) <= lvl * 1.003 and lvl >= close_price * 0.980]

        if nearby_supports:
            support_lvl = max(nearby_supports)
            support_tag = "Pivot P" if support_lvl == p else ("mPOC" if support_lvl == mpoc else "Dip AVWAP")
                ...
                if "GÜÇLÜ AYI" in trend_regime or (is_bear_dump and coin_rs_score < 0.5):
                    ...
                    if not is_buyer_rejection:
                        ...
                        up_targets = [
                            ...
                            if lvl and lvl >= close_price * 1.009
                        ]
                        up_targets.sort()
                        ...
                        await self._handle_open(
                            symbol=symbol, side="LONG", entry_price=close_price,
                            reason=f"{support_tag} Destek Retest Sekmesi (Bullish Support Flip)",
                            soft_stop=soft_stop, hard_stop=hard_stop,
                            tp1=tp1_target, tp2=tp2_target, trade_type="SCALP",
        ...
        # ─────────────────────────────────────────────────────────────────
        break_resists = [lvl for lvl in [tepe_avwap, mvah] if lvl and lvl > 0]
        nearby_break = [lvl for lvl in break_resists if close_price > lvl and current_candle.get('low', close_price) <= lvl * 1.003 and lvl >= close_price * 0.982]

        if nearby_break:
            reclaim_lvl = max(nearby_break)
            reclaim_tag = "Tepe AVWAP" if reclaim_lvl == tepe_avwap else "mVAH"
        ...
        # Fiyat R3 direncini kırmış olup, geri çekilip R3'ü üstten destek olarak test eder
        # ve alıcı tepkisiyle seker → Trend yönünde devam LONG
        # ─────────────────────────────────────────────────────────────────
        if r3 and r3 > 0 and prev_close > r3 and current_candle.get('low', close_price) <= r3 * 1.002 and close_price > r3 and (close_price < r4 if r4 else True):
            struct_ok, struct_reason = self.check_structural_invalidation(symbol, "LONG", close_price, levels)
            if not struct_ok:
                ...
                if not is_buyer_rej:
                    ...
                    await self._handle_open(
                        symbol=symbol, side="LONG", entry_price=close_price,
                        reason="R3 Destek Retest Sekmesi (Bullish Support Flip)",
                        soft_stop=soft_stop, hard_stop=hard_stop,
                        tp1=tp1_target, tp2=tp2_target, trade_type="SCALP",
