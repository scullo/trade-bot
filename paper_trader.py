...
class PaperTrader:
        ...
        try:
            ...
            for attempt in range(1, 6):
                try:
                    ...
                    time.sleep(3 * attempt)  # Artan bekleme: 3s, 6s, 9s, 12s, 15s
            # Tüm denemeler başarısız
            self._last_push_ok = False
            print(">> [GITHUB SYNC KRİTİK] 5 deneme de başarısız! Periyodik sync tekrar deneyecek.")
        finally:
    ...
    def update_tick_telemetry(self, symbol: str, current_price: float):
        ...
        # Tier 1 (Mikro Kâr Kilidi): ROE >= +3.5% -> En az +2.0% ROE kilit (Net Komisyon Üstü Kâr)
        elif current_roe >= 3.5 and not pos.get("_trail_35"):
            pos["_trail_35"] = True
            pos["_trail_be"] = True
            pos["trail_status"] = "🛡️ Tier 1 (%2.0 Kâr Korumalı)"
            if side == "LONG":
                lock_p = round(entry * (1.0 + (0.020 / lev)), 8)
                if lock_p > pos.get("soft_stop", 0):
                    pos["soft_stop"] = lock_p
                    pos["hard_stop"] = lock_p
                    updated_stop = True
    ...
    def open_position(self, symbol: str, side: str, entry_price: float, reason: str, soft_stop: float, hard_stop: float, tp1: float, tp2: float = None, trade_type: str = "BREAKOUT", snapshot_levels: dict = None, setup_id: str = "", confluence_list: list = None, atr_pct: float = 1.0, trend_regime: str = "YATAY", session: str = "LONDRA", volume_surge: float = 1.0, confluence_score: str = "2/4", htf_alignment: str = "TREND YÖNÜNDE", custom_margin: float = None, rs_vs_btc: float = 0.0, decoupling_status: str = "⚪ NÖTR_TAKİPÇİ", cvd_pct: float = 50.0, candle_velocity: float = 1.0, custom_leverage: int = None, **kwargs):
        ...
        pos = {
            ...
            "session": session,
            "session_tag": kwargs.get("session_tag", session),
            "tp1_hit": False,
            ...
            "wick_ratio_pct": float(kwargs.get("wick_ratio_pct", 35.0)),
